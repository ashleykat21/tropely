 ...
 *
 * Each generator returns a "stop" function. Internally it builds an
 * AudioBufferSource (looped) or a chain of oscillators that feed the shared
 * gain node passed in.
 */
...
export type Scene = {
  ...
  emoji: string;
  premium: boolean;
  /** Build the audio nodes; return a teardown function. */
...
export const SCENES: Scene[] = [
  ...
  },
  {
    ...
    premium: true,
    build: (ctx, out) => {
      // Soft pink noise (wind in leaves) + sparse "bird chirp" sine bursts.
  ...
  },
  {
    ...
    premium: true,
    build: (ctx, out) => {
      // Two slowly detuned low sine drones for unease.
...
/** Default scene per mood for auto-sync. */
