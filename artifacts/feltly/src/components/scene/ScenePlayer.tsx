...
export function ScenePlayer() {
  ...
  const effectiveScenes: SceneId[] = useMemo(() => {
    ...
    if (isPremium && autoSync && current) {
      const auto = MOOD_TO_SCENE[current.mood as keyof typeof MOOD_TO_SCENE];
      if (auto) return [auto];
    }
    return [scene];
          ...
          {isPremium && (
            <div className="rounded-xl border border-dashed border-border/60 p-2.5 text-[11px] text-muted-foreground space-y-1.5">
              <div className="flex items-center justify-between gap-2">
                ...
                {mix.length > 0 && (
                  <button onClick={clearMix} className="hover:text-foreground transition">Clear</button>
                )}
              </div>
      ...
      {!open && (
        ...
        >
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (effectiveScenes[0] === "silence" && !playing) setScene("rain");
              setPlaying(!playing);
            }}
