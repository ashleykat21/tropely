...
import { Download, Share2, Sparkles, Copy, Lock } from "lucide-react";
    ...
    try {
      ...
      if (nav.canShare?.({ files: [file] })) {
        await nav.share({
          ...
          text: `My reading mood on Tropely: ${data.tagline}`,
        });
      } else {
        await download();
      }
  ...
  const copyText = async () => {
    try {
      await navigator.clipboard.writeText(
        `My reading mood on Tropely: ${data.tagline}. ${data.finished} books, ${data.pages} pages.`
      );
  ...
  if (!isPremium) {
    return (
      <Button
        ...
        onClick={() =>
          toast("Mood Signature Card is Premium", {
            description: "Generate and share your personalised mood card with Premium.",
            icon: <Sparkles className="h-4 w-4" />,
          })
        }
      >
        <Lock className="h-4 w-4 mr-1.5" /> My mood card
      </Button>
    );
  }
