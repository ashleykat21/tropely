...
import { TROPE_CATEGORIES } from "./tropes";
...
export type Badge = {
  ...
  hint?: string;
  category?: "trope"; // marks trope-specific badges for profile flair
};
...
export function computeBadges(args: {
  ...
  // ── Trope stats ──────────────────────────────────────────────────────────────
  const usedTropes = new Set(books.flatMap((b) => b.tropes ?? []));
  const tropesTagged = usedTropes.size;
  const categoryStats = TROPE_CATEGORIES.map((cat) => ({
    name: cat.name,
    emoji: cat.emoji,
    covered: cat.tropes.filter((t) => usedTropes.has(t)).length,
    total: cat.tropes.length,
  }));
  const anyCompletedCategory = categoryStats.some((c) => c.covered === c.total && c.total > 0);
  const allCategoriesHaveAtLeastOne = categoryStats.every((c) => c.covered > 0);
  const maxCoveredInCategory = Math.max(0, ...categoryStats.map((c) => c.covered));
  ...
  const list: Omit<Badge, "earned">[] = [
    ...
    // ── Trope achievements ────────────────────────────────────────────────────
    // Monthly trope badges
    {
      id: "trope-first", label: "Trope curious", category: "trope",
      desc: "Tag your first trope on any book.",
      emoji: "🎪", progress: Math.min(1, tropesTagged), hint: "Tag a trope on any book.",
    },
    (() => {
      const n = T(3, 2);
      return {
        id: "trope-3", label: "Trope taster", category: "trope" as const,
        desc: `Tag ${n} different tropes.`,
        emoji: "🎠", progress: Math.min(1, tropesTagged / n), hint: `${tropesTagged}/${n} tropes tagged.`,
      };
    })(),
    (() => {
      const n = T(5, 2);
      return {
        id: "trope-5", label: "Trope explorer", category: "trope" as const,
        desc: `Tag ${n} different tropes.`,
        emoji: "🧩", progress: Math.min(1, tropesTagged / n), hint: `${tropesTagged}/${n} tropes tagged.`,
      };
    })(),
    (() => {
      const n = T(3, 2);
      return {
        id: "trope-categories", label: "Category hopper", category: "trope" as const,
        desc: `Tag tropes from ${n} different categories.`,
        emoji: "🗂️",
        progress: Math.min(1, categoryStats.filter((c) => c.covered > 0).length / n),
        hint: `${categoryStats.filter((c) => c.covered > 0).length}/${n} categories.`,
      };
    })(),
    // Monthly: cover at least half the tropes in any one category
    {
      id: "trope-cat-half", label: "Deep dive", category: "trope",
      desc: "Cover at least half the tropes in any one category.",
      emoji: "🏊",
      progress: Math.max(0, ...categoryStats.map((c) =>
        c.total > 0 ? Math.min(1, c.covered / Math.ceil(c.total / 2)) : 0
      )),
      hint: `Best: ${maxCoveredInCategory} tropes in one category.`,
    },

    // Lifetime-scaled trope badges (shown in both; thresholds differ)
    (() => {
      const n = T(10, 2);
      return {
        id: "trope-10", label: "Trope collector", category: "trope" as const,
        desc: `Tag ${n} different tropes.`,
        emoji: "🎭", progress: Math.min(1, tropesTagged / n), hint: `${tropesTagged}/${n} tropes.`,
      };
    })(),
    (() => {
      const n = T(20, 2);
      return {
        id: "trope-20", label: "Trope connoisseur", category: "trope" as const,
        desc: `Tag ${n} different tropes.`,
        emoji: "🎨", progress: Math.min(1, tropesTagged / n), hint: `${tropesTagged}/${n} tropes.`,
      };
    })(),
    (() => {
      const n = T(30, 2);
      return {
        id: "trope-30", label: "Trope master", category: "trope" as const,
        desc: `Tag ${n} different tropes.`,
        emoji: "👑", progress: Math.min(1, tropesTagged / n), hint: `${tropesTagged}/${n} tropes.`,
      };
    })(),
    {
      id: "trope-all-cats", label: "Category roamer", category: "trope",
      desc: "Tag at least one trope from every category.",
      emoji: "🌐",
      progress: Math.min(1, categoryStats.filter((c) => c.covered > 0).length / TROPE_CATEGORIES.length),
      hint: `${categoryStats.filter((c) => c.covered > 0).length}/${TROPE_CATEGORIES.length} categories.`,
    },
    (() => {
      const n = isLifetime ? 50 : 40;
      return {
        id: "trope-50", label: "Trope sage", category: "trope" as const,
        desc: `Tag ${n} different tropes.`,
        emoji: "🌟", progress: Math.min(1, tropesTagged / n), hint: `${tropesTagged}/${n} tropes.`,
      };
    })(),
    {
      id: "trope-cat-completionist", label: "Completionist", category: "trope",
      desc: "Finish every trope in any single category.",
      emoji: "💫",
      progress: anyCompletedCategory ? 1 : Math.max(0, ...categoryStats.map((c) =>
        c.total > 0 ? Math.min(0.99, c.covered / c.total) : 0
      )),
      hint: anyCompletedCategory ? "Done!" : `Best: ${maxCoveredInCategory} tropes in one category.`,
    },
    {
      id: "trope-polyglot", label: "Trope polyglot", category: "trope",
      desc: "Tag at least 3 tropes from every category.",
      emoji: "🔮",
      progress: Math.min(1, categoryStats.filter((c) => c.covered >= 3).length / TROPE_CATEGORIES.length),
      hint: `${categoryStats.filter((c) => c.covered >= 3).length}/${TROPE_CATEGORIES.length} categories with 3+ tropes.`,
    },
  ];

  // Some achievements only make sense as lifetime trophies.
  const lifetimeOnly = new Set(["streak-100", "trope-30", "trope-50", "trope-all-cats", "trope-cat-completionist", "trope-polyglot", "trope-cat-half"]);
...
// The best earned trope-specific badge — used for a dedicated trope profile flair.
export function topEarnedTropeBadge(badges: Badge[]): Badge | null {
  const trope = badges.filter((b) => b.category === "trope");
  for (let i = trope.length - 1; i >= 0; i--) {
    if (trope[i].earned) return trope[i];
  }
  return null;
}
