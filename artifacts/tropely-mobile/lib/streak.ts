...
export function computeStreak(sessions: SessionLog[], freeze?: FreezeState): StreakInfo {
  if (sessions.length === 0) {
    return {
      current: 0, longest: 0, todayPages: 0,
      readToday: false, lastActiveDay: null,
      freezesAvailable: freeze?.available ?? 0, freezeSavedToday: false,
    };
  }
  ...
  for (let i = 1; i < days.length; i++) {
    if (days[i] - days[i - 1] === 86400000) { run++; longest = Math.max(longest, run); }
    else run = 1;
  }
  ...
  let current = 0, freezeSavedToday = false;
...
export function reconcileFreezes(
  ...
  if (!byDay.has(yesterday) && !consumed.has(yesterday) && byDay.has(dayBefore) && state.available > 0) {
    state.available -= 1;
