...
// ─── Spine component ──────────────────────────────────────────────────────────
const TEXT_W = SPINE_H;
const TEXT_H = SPINE_W;
const TEXT_OFF = (SPINE_H - SPINE_W) / 2;
...
function BookSpine({ book, isSelected, onPress }: { book: Book; isSelected: boolean; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity
      onPress={onPress}
      activeOpacity={0.88}
      style={[styles.spine, { backgroundColor: col, shadowColor: col, transform: [{ translateY: isSelected ? -14 : 0 }] }]}
    >
      ...
      <View style={{ width: SPINE_W, height: SPINE_H, overflow: "hidden" }}>
        <View style={{
          ...
          position: "absolute", top: TEXT_OFF, left: -TEXT_OFF,
          transform: [{ rotate: "-90deg" }],
          justifyContent: "center", paddingHorizontal: 8,
        }}>
          <Text style={styles.spineTitle} numberOfLines={1}>{book.title.toUpperCase()}</Text>
...
// ─── Shelf section ────────────────────────────────────────────────────────────
function ShelfSection({ label, count, books, theme, selectedId, onPress, onAddPress }: {
  label: string; count: number; books: Book[];
  theme: (typeof THEMES)[ThemeKey]; selectedId: string | null;
  onPress: (b: Book) => void; onAddPress: () => void;
}) {
  return (
    <View style={{ backgroundColor: theme.bg, marginBottom: 2 }}>
      <Text style={[styles.shelfLabel, { color: theme.title + "90" }]}>
        {label}<Text style={{ color: theme.title + "50" }}> · {count}</Text>
      </Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.spineRow}>
        {books.map((b) => (
          <BookSpine
            key={b.id} book={b} isSelected={selectedId === b.id}
            onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); onPress(b); }}
          />
        ))}
...
// ─── Sort bar (theme-aware) ───────────────────────────────────────────────────
function SortBar({ value, onChange, theme, includeRating }: {
  value: SortKey; onChange: (k: SortKey) => void;
  theme: (typeof THEMES)[ThemeKey]; includeRating?: boolean;
}) {
  ...
  return (
    <View style={{ backgroundColor: theme.bg }}>
      <ScrollView
        ...
        contentContainerStyle={{ gap: 7, paddingHorizontal: 20, paddingVertical: 10 }}
      >
        {opts.map((o) => (
          <TouchableOpacity
            key={o.key} onPress={() => onChange(o.key)}
            style={{
              paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20,
              backgroundColor: value === o.key ? theme.shelf : theme.title + "10",
              borderWidth: 1, borderColor: value === o.key ? theme.edge : theme.title + "28",
            }}
          >
            <Text style={{ fontSize: 11, fontFamily: "Inter_500Medium", color: value === o.key ? theme.title : theme.title + "70" }}>
              {o.label}
            </Text>
          </TouchableOpacity>
        ))}
...
export default function LibraryScreen() {
  ...
  const sortedWant     = sortBooks(want,     sortBy, reflections);
  const sortedFinished = sortBooks(finished, sortBy, reflections);
  const sortedPaused   = sortBooks(paused,   sortBy, reflections);
  ...
  const starRating = selectedBook
    ? (reflections.find((r) => r.bookId === selectedBook.id)?.rating ?? selectedBook.rating)
    : undefined;

  return (
    <View style={[styles.root, { backgroundColor: theme.bg }]}>
      {/* ── Header ── */}
      <View style={[styles.header, { paddingTop: Platform.OS === "web" ? 67 : insets.top + 12, backgroundColor: theme.bg }]}>
        <View>
      ...
      {/* ── Sort bar ── */}
      <SortBar value={sortBy} onChange={setSortBy} theme={theme} includeRating={finished.length > 0} />
      ...
      >
        <ShelfSection
          label="Want to Read" count={sortedWant.length} books={sortedWant}
          theme={theme} selectedId={selectedBook?.id ?? null}
          onPress={openBook} onAddPress={() => router.push("/discover")}
        />
        <ShelfSection
          label="Finished" count={sortedFinished.length} books={sortedFinished}
          theme={theme} selectedId={selectedBook?.id ?? null}
          onPress={openBook} onAddPress={() => router.push("/discover")}
        />
        {sortedPaused.length > 0 && (
          <ShelfSection
            label="Paused" count={sortedPaused.length} books={sortedPaused}
            theme={theme} selectedId={selectedBook?.id ?? null}
            onPress={openBook} onAddPress={() => {}}
          />
        )}
            ...
            <View style={{ flex: 1 }}>
              ...
              {selectedBook.mood && (
                <View style={[styles.moodPill, { backgroundColor: accent + "20", borderColor: accent + "50" }]}>
                  ...
                  <Text style={[styles.moodPillText, { color: accent }]}>{MOODS[selectedBook.mood].label}</Text>
                </View>
              )}

              {/* Star rating for finished books */}
              {selectedBook.shelf === "finished" && starRating ? (
                <View style={{ flexDirection: "row", gap: 3, marginTop: 8 }}>
                  {[1, 2, 3, 4, 5].map((n) => (
                    <Text key={n} style={{ fontSize: 18, color: n <= starRating ? "#F4A628" : colors.border }}>★</Text>
                  ))}
                </View>
              ) : null}

              {/* Progress bar for non-finished books */}
              {selectedBook.pages && selectedBook.pages > 0 && selectedBook.shelf !== "finished" && (
      ...
      <Modal visible={showThemePicker} transparent animationType="slide" onRequestClose={() => setShowThemePicker(false)}>
        <Pressable style={styles.modalBackdrop} onPress={() => setShowThemePicker(false)}>
          <Pressable style={[styles.themeSheet, { backgroundColor: colors.card }]} onPress={() => {}}>
            ...
            <View style={styles.themeGrid}>
              {(Object.keys(THEMES) as ThemeKey[]).map((key) => {
                ...
                return (
                  <TouchableOpacity
                    key={key}
                    style={[styles.themeOption, { backgroundColor: t.bg, borderColor: active ? colors.primary : t.shelf + "CC" }, active && { borderWidth: 2.5 }]}
                    onPress={() => { setShelfTheme(key); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}
                  >
            ...
            <TouchableOpacity style={[styles.themeDone, { backgroundColor: colors.primary }]} onPress={() => setShowThemePicker(false)}>
...
const styles = StyleSheet.create({
  ...
  shelfLabel: { fontSize: 11, fontFamily: "Inter_600SemiBold", letterSpacing: 0.8, textTransform: "uppercase", paddingHorizontal: 20, marginBottom: 10, marginTop: 16 },
  spineRow:   { paddingHorizontal: 20, gap: SPINE_GAP, alignItems: "flex-end", minHeight: SPINE_H + 6 },
  spine:      { width: SPINE_W, height: SPINE_H, borderRadius: 3, overflow: "hidden", shadowOffset: { width: 2, height: 4 }, shadowOpacity: 0.45, shadowRadius: 5, elevation: 5 },
  ...
  addSlot:    { width: SPINE_W, height: SPINE_H, borderRadius: 3, borderWidth: 1.5, borderStyle: "dashed", alignItems: "center", justifyContent: "center" },
  ...
  sheet: { position: "absolute", bottom: 0, left: 0, right: 0, height: SHEET_H, borderTopLeftRadius: 26, borderTopRightRadius: 26, paddingBottom: 24, shadowColor: "#000", shadowOffset: { width: 0, height: -6 }, shadowOpacity: 0.35, shadowRadius: 20, elevation: 24 },
  ...
  sheetCover:     { width: 82, height: 118, borderRadius: 8, borderWidth: 1, alignItems: "center", justifyContent: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.22, shadowRadius: 8 },
  ...
  moodPill:       { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, borderWidth: 1, alignSelf: "flex-start" },
  ...
  sheetSecondary: { borderRadius: 14, paddingVertical: 12, alignItems: "center", borderWidth: 1, flexDirection: "row", gap: 8, justifyContent: "center" },
  ...
  themeOption:   { width: "44%", paddingVertical: 18, paddingHorizontal: 12, borderRadius: 18, alignItems: "center", borderWidth: 1.5, overflow: "hidden" },
  ...
  activeBadge:   { position: "absolute", top: 8, right: 8, width: 20, height: 20, borderRadius: 10, alignItems: "center", justifyContent: "center" },
