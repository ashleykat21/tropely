  ...
  return (
    <View style={[s.milestone, { borderLeftColor: col }]}>
      <Pressable style={s.milestoneHead} onPress={onToggle}>
        <View style={{ flex: 1 }}>
          <View style={s.milestoneLabelRow}>
            ...
            {m.done && <Tag label="✓ Done" color={C.green} />}
          </View>
          <Text style={s.milestoneTitle}>{m.title}</Text>
          <Text style={s.milestoneSub}>{m.chapters} · Due {m.deadline}</Text>
        </View>
        <View style={s.milestoneRight}>
          <View style={[s.deadlinePill, { borderColor: col+"55" }]}>
      ...
      {expanded && (
        <View style={s.chList}>
          {m.data.length === 0
            ? <View style={s.emptyMilestone}><Text style={s.emptyTxt}>Chapters unlocked when you reach this milestone</Text></View>
        ...
        return (
          <Pressable key={i} style={s.pollOption} onPress={() => setVoted(i)}>
            <View style={s.pollRow}>
              <Text style={[s.pollLabel, active && { color: C.primary, fontWeight: "700" }]}>{o.label}</Text>
              <Text style={[s.pollPct, active && { color: C.primary }]}>{o.pct}%</Text>
            </View>
...
export default function BuddyReadsScreen() {
  ...
  return (
    <View style={[s.root, { paddingTop: Platform.OS==="web" ? 67 : insets.top }]}>
      ...
      <View style={s.topBar}>
        <View style={[s.headerIcon, { backgroundColor: C.primary+"25" }]}>
...
const s = StyleSheet.create({
  ...
  addBtn: { width: 34, height: 34, borderRadius: 17, backgroundColor: C.card, borderWidth: 1, borderColor: C.border, alignItems: "center", justifyContent: "center" },
